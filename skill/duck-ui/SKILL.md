...
7. **Compose with shadcn.** duck/ui is additive. For a table, a menu, a combobox or anything else it does not ship, use standard shadcn/ui — the theme already styles it. The overlays duck does ship are the ones the sticker edge changes: `StickerDialog`, `StickerDrawer`, `StickerPopover`, `StickerTooltip` and `DuckCommand`. Colour, radius and type reach a stock overlay on their own; the 3px edge does not, so paste `STICKER_SURFACE` — exported by `@duck/sticker-popover` — onto a `DropdownMenuContent` that opens beside a sticker surface: `className={cn(STICKER_SURFACE, "p-1")}`. A select is the exception that proved the rule and now has its own item, `@duck/glow-select`: a stock trigger beside a `GlowInput` reads as a different design system, and the field family is the one place a 1px border cannot pass.
...
9. **Turning the edge off is `frame={false}`, never `border-0`.** `.sticker` is declared at the end of the utilities layer, so a border utility at the call site loses on order at equal specificity and the 3px edge stays. Every component that draws the edge takes `frame` — `GlowInput`, `GlowTextarea`, `GlowSelect`, `GlowColor`, `StickerCard`, `HudChip`, `DuckTabsList`, `StickerKbd`. On anything else, including a stock element wearing a pasted recipe, use the `sticker-none` utility, or `[--sticker-width:1px]` for a hairline: `.sticker` reads its width through that variable, so a call site wins whatever the source order.
...
| Component | Key props |
|---|---|
| `QuackButton` | `variant: primary \| holo \| outline \| ghost \| danger`, `size: xs \| sm \| default \| lg \| icon-xs \| icon-sm \| icon` (`xs`/`icon-xs`/`icon-sm` are the 28–32px instrument scale a control rail runs on, radius and icon stepped down with them; for an exact 24px control take `icon-xs` and add `size-6`), `idle: none \| breathe \| sheen \| pulse \| float`, `magnetic: number`, `ripple: boolean`, `state: idle \| loading \| success \| error`, `loadingLabel`, `successLabel`, `errorLabel`, `asChild` |
| `HoloButton` | `variant: holo \| primary \| outline \| ghost`, `size`, `asChild` |
| `CopyButton` | `value`, `label`, `copiedLabel`, `errorLabel`, `onCopied`, `onError`. A refused write — plain HTTP, an embedded browser, a denied prompt — shows the cross, announces `errorLabel` and calls `onError`; it never claims a success it did not get |
...
| `HudChip` | `variant: outline \| ghost \| primary`, `size: xs \| sm \| default` (24 / 28 / 36px), `active`, `asChild`, `frame`, plus native button props. The interactive `HudLabel`: mono, uppercase, tracked, from the `.hud` utility rather than the `--*-button` tokens, so it reads as machine output and not as a CTA. `active` is visual and emits `data-active` — add `aria-current` / `aria-pressed` / `aria-selected` at the call site, since the same highlight means all three. Needs `@duck/hud-label` for `.hud` |
| `StickerCard` (+ `Header`, `Title`, `Description`, `Content`, `Footer`) | `holo`, `tilt`, `peel`, `ticks`, `glass`, `frame`, `asChild`. `frame={false}` keeps the fill, the radius and the padding for a card nested inside a surface that is already the frame |
...
| `HoloBadge` | `variant: holo \| primary \| outline \| muted \| success \| danger`, `shape: pill \| tag \| block`, `asChild`. `block` is the rail-wide status strip — full width, centred — rather than a tag plus `w-full justify-center` |
| `HudLabel` | `tone: muted \| foreground \| primary \| accent`, `size: sm \| default`, `tracking: default \| tight`, `dot`, `dotTone: muted \| foreground \| primary \| accent \| destructive`, `asChild` (a rail heading that is also an `h3`). The dot takes the label's colour and only `primary` glows; use `dotTone` for a red dot on a muted row, never a `[&>span]` selector. Also ships a `.hud` utility for labels that are an attribute of an existing element |
...
| `StickerKbd` | `watch` (a KeyboardEvent.key), `meta`, `frame`. `watch` is teaching rather than decoration: a dock or a palette that prints its own keyboard map presses the cap under the user's finger. `frame={false}` prints the cap flat for a keycap inside a tooltip or a menu row |
...
| `GlowSelect` (+ `Root`, `Trigger`, `Value`, `Content`, `Item`, `Group`, `Label`, `Separator`) | `value` / `defaultValue` / `onValueChange`, `placeholder`, `size: sm \| default`, `frame`, `chevron`, `contentClassName`; `className` styles the trigger. Radix Select underneath — typeahead, roving highlight, collision-aware positioning, and a hidden native select that still submits. The trigger wears `GlowInput`'s exported class strings and the menu wears `STICKER_SURFACE`, both imported, so a select beside a field cannot drift from it. Inside `GlowField` pass nothing extra: the cloned `id`, `aria-describedby` and `aria-invalid` land on the trigger |
| `GlowColor` | `value` / `defaultValue` / `onValueChange` (always `#rrggbb`), `size: sm \| default`, `eyedropper` (default `true`, Chromium only, rendered after mount), `showValue`, `frame`, `swatchClassName`. The colour field: the four rules that stop `input[type=color]` looking like an OS widget live in the component. A picked colour has no DOM event, so wire `onValueChange`, not `onChange`. No alpha channel — an eighth and ninth hex digit are dropped |
...
| `DuckSlider` | `value`, `defaultValue`, `min`, `max`, `step`, `formatValue`, `showValue`, `valuePosition: above \| row`, `label`, `action`, `curve: linear \| log`, `onValueChange`. Single value, no range. `curve="log"` owns the geometric mapping a font-size (12→400) or zoom (0.05→8) slider needs and keeps `aria-valuetext` honest — read the value from `onValueChange`, since the input's own `valueAsNumber` is a track position there. One arrow press is one `step` anywhere on a log track, because a position is worth less than a step over most of a wide range. `label` + `valuePosition="row"` + `action` is the control-rail row: name, readout and reset on one line that a drag cannot reflow |
...
| `DuckTabs` (+ `List`, `Trigger`, `Content`) | `value`, `defaultValue`, `onValueChange`, `orientation: horizontal \| vertical`; the list also takes `frame`. Vertical is a real axis change — the list measures the vertical axis, Up and Down move, `aria-orientation` says so, and the indicator becomes a bar down the left edge rather than a pill sized to the longest label |
...
| `StickerTooltip` (+ `Provider`, `Root`, `Trigger`, `Content`) | `content`, `side: top \| right \| bottom \| left`, `align: start \| center \| end`, `arrow`, `delay` (ms, default `250`), `wrapDisabled` (default `true`), `children` as the trigger, so it must forward props. Radix Tooltip with the die-cut edge. A disabled child is wrapped in a `tabIndex={0}` span, because a dead control receives no pointer events and no focus — and "why can't I press Save?" is the case a tooltip exists for. Hover-only chrome: no touch, gone as soon as the pointer leaves, so use it for a label with nowhere else to live and never for anything the task needs — a shortcut goes inline in `StickerKbd`. The all-in-one brings its own provider; compose the parts when a row of controls should share one delay |
...
| `DuckFaq` | `@/components/blocks/duck-faq` | `items: { question, answer, answerText? }[]`, `title`, `description`, `columns: 1 \| 2`, `collapsible`, `defaultOpen`, `jsonLd` (default `true`), `url`, `footer`. The section and its `FAQPage` JSON-LD come off the same array. Pass `answerText` when the answer is JSX — that is what the schema carries |
| `DuckLogoWall` | `@/components/blocks/duck-logo-wall` | `logos: { name, src?, node?, href? }[]`, `eyebrow`, `title`, `variant: grid \| marquee`, `logoHeight` (px, one number for the whole set), `tone: mono \| colour`, `duration`, `footer`. A logo with no `src` or `node` renders its name as a wordmark |
| `DuckCtaBand` | `@/components/blocks/duck-cta-band` | `title`, `description`, `eyebrow`, `primaryAction`, `secondaryAction`, `capture: { action, method?, name?, type?, placeholder?, label?, buttonLabel? }`, `note`, `variant: holo \| solid`, `align: center \| split`. `capture.action` takes a URL or a server function, so the same block posts on Vite and on Next |
| `DuckWorkbench` | `@/components/blocks/duck-workbench` | `tools: { icon, label, active?, disabled?, onSelect? }[]`, `title`, `actions`, `inspector` + `inspectorLabel` + `inspectorWidth`, `status`, `grid`, `viewportProps`, `viewportRef`, `children` (the canvas content). Editor shell: tool rail, pan-and-zoom canvas, inspector, status strip |
| `DuckChatThread` | `@/components/blocks/duck-chat-thread` | `messages: { id, from: assistant \| user, content, meta?, streaming? }[]`, `onSend`, `busy`, `thinking` + `thinkingLabel`, `mark`, `header`, `empty`, `placeholder`, `sendLabel`, `jumpLabel`, `composerActions`, `note`, `maxRows` |
| `DuckMediaShelf` | `@/components/blocks/duck-media-shelf` | `rows: { title?, description?, actions?, items, aspect?, tileWidth?, loading?, emptyHint? }[]`, item is `{ id, title?, subtitle?, src?, alt?, href?, progress?, overlay? }`; plus `aspect`, `tileWidth`, `skeletonCount`, `empty`, `render`. One tile width for the whole wall |
| `DuckPlayerBar` | `@/components/blocks/duck-player-bar` | `art`, `title`, `subtitle`, `duration`, `position`, `buffered`, `playing` + `onPlayingChange`, `onSeek` (commit) / `onScrub` (live), `onPrevious`, `onNext`, `volume` + `onVolumeChange`, `muted` + `onMutedChange`, `preview`, `actions`, `shortcuts` (opt-in, leaves text fields alone), `seekStep` |
| `DuckListView` | `@/components/blocks/duck-list-view` | `columns`, `rows: { id, title?, description?, meta?, index?, cells?, values?, href?, trailing? }[]`, `title`, `actions`, `search` + `searchPlaceholder` + `onSearch`, `sort` + `onSortChange`, `loading` + `skeletonRows`, `empty`, `noMatchesTitle`, `showCount`, `render`. Sorting compares `values`, never `cells` |
| `DuckSettingsPanel` | `@/components/blocks/duck-settings-panel` | `sections: { title?, description?, rows, actions? }[]`, row is `{ label, control, description?, labelling: control \| group, hint?, stacked? }`; plus `title`, `description`, `dirty`, `saving`, `saveLabel`, `resetLabel`, `onReset`, `footer`, `variant: plain \| cards` |
| `DuckAuthCard` | `@/components/blocks/duck-auth-card` | `onRequestCode`, `onVerify`, `brand`, `title`, `description`, `providers: { label, icon?, onSelect? }[]`, `codeLength`, `resendIn`, `submitOnComplete`, `error`, `footer`, `legal`, `emailLabel`, `submitLabel`, `verifyLabel` |
| `DuckUpload` | `@/components/blocks/duck-upload` | `onUpload: (file, { onProgress, signal }) => Promise<unknown>`, `onQueueChange`, `onSettled`, `accept`, `multiple`, `maxSize`, `concurrency`, `label`, `hint`, `clearDoneAfter`. The block owns the queue; `onUpload` owns the network |
| `DuckChangelog` | `@/components/blocks/duck-changelog` | `releases: { version, date?, title?, tags?, highlights?, body? }[]`, `title`, `description`, `eyebrow`, `idPrefix`, `locale`, `latestLabel`, `headingLevel: h2 \| h3`. The anchor is the slugified version, so `#1-2-0` is linkable |
...
- `DuckCtaBand` may take the holo edge even on a page whose hero already spent it — the budget is per viewport, not per page. What it may not do is put holo on the band *and* its button, so a holo band flips its primary action to lime.
- `DuckWorkbench`'s zoom controls are a sibling of the viewport, never a child: a child sits inside the transform and pans away with the content. The dot grid is the opposite — inside the transform, because it is the paper.
- `DuckChatThread` follows the stream only while the reader is already at the bottom, and offers a jump button otherwise. Never scroll a thread the reader has scrolled away from.
- `DuckListView` keeps empty and no-matches as different screens. "Nothing here yet" wants an action; "nothing matched hedgehog" wants the query cleared. Give rows `values` or sorting compares JSX and sorts by nothing.
- `DuckSettingsPanel` rows are labels, so the block never mints an id for a control it does not own. A row whose control is plural takes `labelling="group"` — a label may only point at one field.
- `DuckUpload` treats an abort as a decision, not a failure: the row reads *Canceled* and offers retry rather than turning red.
- `DuckFaq` emits one `FAQPage` per URL. Two FAQ sections on one page means `jsonLd={false}` on the second.
...
Utility classes: `.holo-border`, `.holo-border-animated`, `.holo-text`, `.foil`, `.sheen`, `.tilt`, `.duck-glow`, `.duck-glow-primary`, `.duck-stream-edge`, `.sticker`, `.sticker-none`, `.holo-edge`, `.kiss-cut`, `.cut-line`.

`.sticker` reads its width from `--sticker-width`, falling back to `--sticker-border`, which is what makes the edge overridable from a call site that would otherwise lose on source order — see design rule 9. The theme declares no typefaces: `--font-sans` and `--font-mono` stay Tailwind's, and `--font-display` follows `--font-sans` until a project points it at a display face. `@duck/theme-noir` is the exception and does set all three.
